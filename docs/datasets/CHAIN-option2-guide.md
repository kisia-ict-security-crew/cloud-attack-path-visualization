...
## 8. 노이즈 병행 — 같은 엔티티를 정상 활동으로도 등장시키기

무관한 로그를 섞는 게 아니라, **공격이 건드리는 그 엔티티(인스턴스·크리덴셜·victim 버킷·트레일)를 정상 활동도 건드리게** 해서, "그 엔티티 접근 = 공격"으로 못 거르게 만든다. 판별 기준이 엔티티가 아니라 **행위 패턴**이 된다.

### 스크립트 3종 (같은 폴더)

| 파일 | 역할 | 겹치는 엔티티 |
|---|---|---|
| `noise_instance.sh` | 인스턴스가 **자기 역할 크리덴셜(ASIA)**로 정상 호출 (SSM 루프) | 크리덴셜 — **같은 키, 인스턴스 IP** baseline |
| `noise_ops.sh` | 정상 운영자가 describe/list/get/put | 인스턴스·victim 버킷·IAM·트레일 |
| `chain_with_noise.sh` | 위 둘을 백그라운드로 돌리며 그 사이에 체인 자동 실행 | 전부 |

### 핵심 효과 — "같은 크리덴셜, 다른 IP"
`noise_instance.sh`가 인스턴스 IP에서 ASIA 키로 정상 호출을 계속 쌓는 동안, 3절의 탈취 체인은 **같은 ASIA 키를 공격자 IP에서** 쓴다. 그래프에서 한 크리덴셜 노드에 매달린 이벤트가 소스 IP로 갈라진다 — v3가 ground-truth로 검증한 바로 그 탐지 형태다.

### 실행 (병행)
```bash
export OPS_PROFILE=<정상운영프로파일>     # 노이즈 B용 정상 신원 (미설정 시 default)
export AWS_DEFAULT_REGION=ap-northeast-2
# Phase A(2절) 완료 상태에서:
./chain_with_noise.sh
```
- pre-roll(정상만) → 공격 5단계 → post-roll(정상만) 순으로 진행돼, 공격이 정상 트래픽 **중간에 묻힌다**.
- 세기 조절: `NOISE_DURATION`, `OPS_INTERVAL`(작을수록 진함), `INST_INTERVAL`.
- 대조군: `NO_NOISE=1 ./chain_with_noise.sh`(깨끗한 체인), `NO_ATTACK=1 ./chain_with_noise.sh`(정상만).

### 실행 (수동으로 단계 사이에 삽입)
자동화 대신 3절을 손으로 하며 단계 사이에 노이즈 버스트를 끼우려면:
```bash
# 단계 사이에 30초짜리 정상 트래픽 삽입
AWS_PROFILE=ops ./noise_ops.sh "$INSTANCE_ID" "$VICTIM_BUCKET" "$DECOY_TRAIL" 30 3
```

### 데이터셋 3종 만들기 (난이도별)
같은 절차로 SNR만 바꿔 세 벌을 뽑으면 그래프의 판별력을 단계별로 평가할 수 있다.
| 데이터셋 | 명령 | 용도 |
|---|---|---|
| 깨끗한 체인 | `NO_NOISE=1 ./chain_with_noise.sh` | 그래프가 경로를 **잇는가** |
| 중간 노이즈 | `OPS_INTERVAL=8 ./chain_with_noise.sh` | 판별 난이도 중 |
| 진한 노이즈 | `OPS_INTERVAL=2 ./chain_with_noise.sh` | 판별 난이도 상 |
